...
const followUpBtn = document.querySelector("#follow-up");
...
//send follow-up button that will send a follow-up message to Claude API
followUpBtn.addEventListener("click", sendFollowUpMessage);
...
// function to send FollowUpMessages
function sendFollowUpMessage() {
    let followUpText = userMessage.value.trim();
    ...
    let url = `${baseURL}/api/claude/messages`;
    // Prepare the request body for the follow-up message
    let requestBody = {
        max_tokens: maxTokens,
        messages: conversationHistory,
        model: "claude-sonnet-5"
    ...
    //  Make the API request using fetch() for follow-up message
    fetch(url, {
        method: "POST",
        headers: {
            "X-Student-API-Key": studentApiKey,
    ...
    .then(json => {
        ...
        // Add Claude's follow-up response to conversation history
        conversationHistory.push({
    role: "assistant",
    content: json.content[0].text
});
        ...
        displayMessage(json, true);
    })
    .catch(err => console.error("Follow-up error:", err));
}


function displayMessage(json,isFollowUp){
    console.log(json);
    ...
    if(!isFollowUp){
        // STEP 8g: Display Claude's response in the results section
        para.style.background = "rgb(34, 34, 34)";
        para.textContent = `Assistant first response: ${json.content[0].text}`;
    }else{
        para.style.background = "rgb(0, 68, 136)";
        para.textContent = `Assistant follow-up response: ${json.content[0].text}`;
    }
