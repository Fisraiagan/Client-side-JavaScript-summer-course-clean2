async function startProgram() {
    // The actions we used in the previous block of code were METHODS -
    //  functions that are attached to an object (the robot, in this case). 
    // FUNCTIONS are standalone blocks of code that can be called from anywhere in the program.
    //  They are reusable and can take parameters to customize their behavior.
...
// STEP 1: Create a FUNCTION called celebrate (note that we are outside of the main program block)
    ...
    // STEP 2: Set the main LED color to red
       await setMainLed({r:200, g:0,b:0});
    // STEP 3: Roll the robot
        await roll(0,50 ,8);
    // STEP 4: Set the main LED color to blue
        await setMainLed({ r: 0, g: 0, b: 200 });
